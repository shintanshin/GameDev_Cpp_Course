...
namespace UI
{
	...
	std::string getTribeString(Tribe tribe)
	{
		...
		case Tribe::Amorphous: return "AMORPHOUS";
		case Tribe::God: return "GOD";
		case Tribe::Dark_Creature: return "DARK CREATURE";
		case Tribe::Count:
		default:
			return "";
		}
	...
	void printMonsterInfo(Monster* monster)
	{
		std::cout << "\n--------Monster \"" << monster->getName() << "\"" << ", of " <<
			...
			"-------- BAD THINGS: " << monster->getRunawayPolicy()->getFullInfo() << " ------" << std::endl << 
			"-------- REWARDS: " << monster->getVictoryPolicy()->getFullInfo() << " ------" << std::endl << std::endl;
	}
...
{
	return m_monstersDeck.generateMonster(); // base code
}